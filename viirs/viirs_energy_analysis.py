#!/usr/bin/env python3
"""
Comprehensive Ocean Thermal Energy Analysis
Calculates energy trends, integrated energy, degree-days, and more from VIIRS SST data
"""
...
def plot_comprehensive_dashboard(df, results_df, output_file='energy_dashboard.png', figsize=(18, 12)):
    ...
    width = 0.35
    ax3.bar(x - width/2, results_sorted['heating_dd_yr'], width,
            label='Heating', color='red', alpha=0.7)
    ax3.bar(x + width/2, results_sorted['cooling_dd_yr'], width,
            label='Cooling', color='blue', alpha=0.7)
    ax3.set_xticks(x)
    ax3.set_xticklabels(results_sorted['region'], rotation=45, ha='right', fontsize=8)
    ax3.set_ylabel('Degree-Days per Year', fontsize=10)
    ax3.set_title('Heating vs Cooling', fontsize=11, fontweight='bold')
    ...
    # Plot 5: Extreme Events
    ax5 = fig.add_subplot(gs[1, 1])
    ax5.barh(results_sorted['region'], results_sorted['pct_time_extreme'],
             color='darkred', alpha=0.7)
    ax5.set_xlabel('% Time Above 95th Percentile', fontsize=10)
    ax5.set_title('Extreme Energy Events', fontsize=11, fontweight='bold')
    ...
    # Plot 7: Year-over-Year Trend
    ax7 = fig.add_subplot(gs[2, 0])
    yoy_data = results_sorted['yoy_integrated_trend_GJ_days_m2_yr2'].dropna()
    if len(yoy_data) > 0:
        yoy_regions = results_sorted.loc[yoy_data.index, 'region']
        colors7 = ['green' if x > 0 else 'red' for x in yoy_data]
        ax7.barh(yoy_regions, yoy_data, color=colors7, alpha=0.7)
        ax7.axvline(x=0, color='black', linestyle='-', linewidth=0.8)
        ax7.set_xlabel('YoY Trend (GJ·days/m²/yr²)', fontsize=10)
        ax7.set_title('Year-over-Year Trend', fontsize=11, fontweight='bold')
    ...
    fig.suptitle('Comprehensive Ocean Thermal Energy Analysis Dashboard',
...
def print_summary(results_df):
    ...
    # Strongest trends (check for valid data)
    if results_df['sst_trend_C_yr'].notna().any():
        ...
        print("TEMPERATURE TRENDS: Insufficient data")
    ...
    if results_df['annual_integrated_GJ_days_m2_yr'].notna().any():
        ...
        print("\n\nENERGY ACCUMULATION: Insufficient data")

    # Gulf comparisons
    gsv_regions = results_df[results_df['region'].str.contains('SVG|Gulf St Vincent', case=False, na=False)]
    sg_regions = results_df[results_df['region'].str.contains('Spencer', case=False, na=False)]

    if len(gsv_regions) > 0 and len(sg_regions) > 0:
        print("\n\nGULF COMPARISON:")
        print(f"  Gulf St Vincent:")
        print(f"    Avg SST trend: {gsv_regions['sst_trend_C_yr'].mean():+.4f} °C/yr")
        print(f"    Avg integrated energy: {gsv_regions['annual_integrated_GJ_days_m2_yr'].mean():.2f} GJ·days/m²/yr")

        print(f"\n  Spencer Gulf:")
        print(f"    Avg SST trend: {sg_regions['sst_trend_C_yr'].mean():+.4f} °C/yr")
        print(f"    Avg integrated energy: {sg_regions['annual_integrated_GJ_days_m2_yr'].mean():.2f} GJ·days/m²/yr")
    elif len(gsv_regions) > 0:
        print("\n\nGULF COMPARISON:")
        print(f"  Gulf St Vincent:")
        print(f"    Avg SST trend: {gsv_regions['sst_trend_C_yr'].mean():+.4f} °C/yr")
        print(f"    Avg integrated energy: {gsv_regions['annual_integrated_GJ_days_m2_yr'].mean():.2f} GJ·days/m²/yr")
        print(f"\n  Spencer Gulf: No data available")
    elif len(sg_regions) > 0:
        print("\n\nGULF COMPARISON:")
        print(f"  Gulf St Vincent: No data available")
        print(f"\n  Spencer Gulf:")
        print(f"    Avg SST trend: {sg_regions['sst_trend_C_yr'].mean():+.4f} °C/yr")
        print(f"    Avg integrated energy: {sg_regions['annual_integrated_GJ_days_m2_yr'].mean():.2f} GJ·days/m²/yr")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='VIIRS SST Data - Comprehensive Thermal Energy Analysis',
